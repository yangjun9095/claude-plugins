...
def _rp(p):
    """Expected values must be realpath'd: git reports physically-resolved
    paths, and CI runs on macOS where /tmp -> /private/tmp."""
...
def test_pure_and_subprocess_agree_from_main_worktree(repo_with_worktrees):
    ...
    assert anchor.git_common_dir(str(main)) == os.path.join(_rp(main), ".git")
...
def test_pure_and_subprocess_agree_from_linked_worktree(repo_with_worktrees):
    ...
    for wt in wts:
        ...
        assert anchor.git_common_dir(str(wt)) == os.path.join(_rp(main), ".git")
...
def test_agree_from_subdirectory_of_a_worktree(repo_with_worktrees):
    ...
    assert got == os.path.join(_rp(main), ".git")


def test_bare_repo_common_dir_is_the_bare_dir_itself(bare_repo_with_worktree):
    bare, wt = bare_repo_with_worktree
    assert anchor.git_common_dir(str(bare)) == _rp(bare)
    assert anchor.git_common_dir_pure(str(bare)) == _rp(bare)
...
def test_threads_dir_is_inside_the_common_dir(repo_with_worktrees):
    main, wts = repo_with_worktrees
    expected = os.path.join(_rp(main), ".git", "agent-board")
...
def test_submodule_resolves_to_the_superproject(repo_with_submodule):
    ...
    expected = os.path.join(_rp(outer), ".git")
...
def test_resolvers_agree_when_dot_git_is_itself_a_symlink(repo_with_worktrees, tmp_path):
    """A relocated-and-symlinked .git is a normal setup."""
    main, _ = repo_with_worktrees
    moved = tmp_path / "moved_gitdir"
    dot = os.path.join(str(main), ".git")
    os.rename(dot, str(moved))
    os.symlink(str(moved), dot)
    got = anchor.git_common_dir_pure(str(main))
    assert got == anchor.git_common_dir(str(main))
    assert got == _rp(moved)


def test_resolvers_agree_when_gitdir_pointer_traverses_a_symlink(
        repo_with_worktrees, tmp_path):
    """A linked worktree's .git FILE records a path; if any component of that
    recorded path is a symlink, an abspath-based resolver keeps the link while
    git resolves it. This is the linked-worktree/submodule shape."""
    main, wts = repo_with_worktrees
    wt = wts[0]
    real_common = os.path.join(_rp(main), ".git")
    link_root = tmp_path / "link_to_main"
    os.symlink(str(main), str(link_root))
    via_link = os.path.join(str(link_root), ".git", "worktrees",
                            os.path.basename(str(wt)))
    with open(os.path.join(str(wt), ".git"), "w") as fh:
        fh.write("gitdir: %s\n" % via_link)
    got = anchor.git_common_dir_pure(str(wt))
    assert got == anchor.git_common_dir(str(wt))
    assert got == real_common
